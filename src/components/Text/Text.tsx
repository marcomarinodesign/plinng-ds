import { type ElementType, type HTMLAttributes } from "react";
import { clsx } from "clsx";
import { twMerge } from "tailwind-merge";

export type TextVariant =
  // Hero — responsive (mobile/desktop), always bold
  | "hero-xl"              // Desktop 80px w800 · Mobile 60px w800
  | "hero-lg"              // Desktop 60px w700 · Mobile 45px w700
  | "hero-md"              // Desktop 48px w700 · Mobile 36px w700
  | "hero-sm"              // Desktop 34px w700 · Mobile 30px w700
  // Header — responsive (mobile/desktop), always bold, confirmed ✓
  | "header-h1"            // Desktop 30px · Mobile 28px
  | "header-h2"            // Desktop 28px · Mobile 26px
  | "header-h3"            // 24px
  | "header-h4"            // 22px
  // Subhead — confirmed ✓
  | "subhead"              // 20px, regular
  | "subhead-bold"         // 20px, bold
  // Body — confirmed ✓
  | "body-editorial"       // 18px, regular
  | "body-editorial-bold"  // 18px, bold
  | "body-lg"              // 16px, regular
  | "body-lg-bold"         // 16px, bold
  | "body-sm"              // 14px, regular
  // Utility — confirmed ✓
  | "overline"             // 12px · bold · uppercase · ls 2px
  | "caption";             // 12px, regular

export interface TextProps extends HTMLAttributes<HTMLElement> {
  variant?: TextVariant;
  as?: ElementType;
  bold?: boolean;
}

const defaultElement: Record<TextVariant, ElementType> = {
  "hero-xl":             "h1",
  "hero-lg":             "h2",
  "hero-md":             "h2",
  "hero-sm":             "h2",
  "header-h1":           "h1",
  "header-h2":           "h2",
  "header-h3":           "h3",
  "header-h4":           "h4",
  "subhead":             "h5",
  "subhead-bold":        "h5",
  "body-editorial":      "p",
  "body-editorial-bold": "p",
  "body-lg":             "p",
  "body-lg-bold":        "p",
  "body-sm":             "p",
  "overline":            "span",
  "caption":             "span",
};

// Base classes — mobile-first, desktop via md: prefix. Values confirmed from Figma ✓
const variantClasses: Record<TextVariant, string> = {
  // Hero — tracking-tight, responsive
  "hero-xl":             "text-[60px] leading-[70px] tracking-[-1.2px]  font-extrabold md:text-[80px] md:leading-[90px]  md:tracking-[-1.6px]",
  "hero-lg":             "text-[45px] leading-[55px] tracking-[-0.9px]  font-bold      md:text-[60px] md:leading-[70px]  md:tracking-[-1.2px]",
  "hero-md":             "text-[36px] leading-[46px] tracking-[-0.72px] font-bold      md:text-[48px] md:leading-[58px]  md:tracking-[-0.96px]",
  "hero-sm":             "text-[30px] leading-[40px] tracking-[-0.6px]  font-bold      md:text-[34px] md:leading-[44px]  md:tracking-[-0.68px]",
  // Header — confirmed ✓, responsive for h1/h2
  "header-h1":           "text-[28px] leading-[34px] font-bold md:text-[30px] md:leading-[36px]",
  "header-h2":           "text-[26px] leading-[32px] font-bold md:text-[28px] md:leading-[34px]",
  "header-h3":           "text-[24px] leading-[28px] font-bold",
  "header-h4":           "text-[22px] leading-[26px] font-bold",
  // Subhead — confirmed ✓
  "subhead":             "text-[20px] leading-[28px] font-normal",
  "subhead-bold":        "text-[20px] leading-[28px] font-bold",
  // Body — confirmed ✓
  "body-editorial":      "text-[18px] leading-[26px] font-normal",
  "body-editorial-bold": "text-[18px] leading-[26px] font-bold",
  "body-lg":             "text-[16px] leading-[22px] font-normal",
  "body-lg-bold":        "text-[16px] leading-[22px] font-bold",
  "body-sm":             "text-[14px] leading-[20px] font-normal",
  // Utility — confirmed ✓
  "overline":            "text-[12px] leading-[16px] font-bold uppercase tracking-[2px]",
  "caption":             "text-[12px] leading-[16px] font-normal",
};

export function Text({
  variant = "body-lg",
  as,
  bold,
  children,
  className,
  ...rest
}: TextProps) {
  const Tag = as ?? defaultElement[variant];

  return (
    <Tag
      className={twMerge(
        clsx(
          "font-sans text-primary",
          variantClasses[variant],
          bold && "font-bold",
          className,
        ),
      )}
      {...rest}
    >
      {children}
    </Tag>
  );
}
